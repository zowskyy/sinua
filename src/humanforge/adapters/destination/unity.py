"""Unity destination adapter — writes an HF sidecar JSON for Unity.

The Unity plugin reads this sidecar and drives character animation via
the Humanoid Rig or a custom component. No Unity dependencies required.
"""
...
class UnityDestinationAdapter(DestinationAdapter):
    ADAPTER_ID = "hf.destination.unity.v1"
    ADAPTER_VERSION = "1.0.0"
    DESTINATION_TYPE = "unity"
    ...
    def export(
        ...
        *,
        clip_name: str = "HFPerformance",
        **kwargs,
    ) -> Path:
        path = Path(output_path)
        if path.suffix.lower() != ".json":
            path = path.with_suffix(".hfunity.json")
        ...
        sidecar = {
            "format": "humanforge-unity-sidecar",
            "format_version": "1.0",
    ...
    def validate_export(
        ...
        checks.append(ExportCheckResult(
            ...
            passed=path.exists(),
            severity="error",
            message=f"Unity sidecar not found: {path}",
        ))
        ...
        if path.exists():
            try:
                ...
                checks.append(ExportCheckResult(
                    name="correct_format",
                    passed=data.get("format") == "humanforge-unity-sidecar",
                    severity="error",
...
register_destination(UnityDestinationAdapter())
