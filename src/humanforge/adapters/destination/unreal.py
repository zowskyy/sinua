"""Unreal Engine destination adapter — writes an HF sidecar JSON for UE5.

The Unreal plugin reads this sidecar and applies animation via Live Link
or a blueprint function library. No Unreal dependencies are required here.
"""
...
class UnrealDestinationAdapter(DestinationAdapter):
    ADAPTER_ID = "hf.destination.unreal.v1"
    ADAPTER_VERSION = "1.0.0"
    DESTINATION_TYPE = "unreal"
    ...
    def export(
        ...
        *,
        asset_name: str = "HFPerformance",
        **kwargs,
    ) -> Path:
        path = Path(output_path)
        if path.suffix.lower() != ".json":
            path = path.with_suffix(".hfue.json")
        ...
        sidecar = {
            "format": "humanforge-unreal-sidecar",
            "format_version": "1.0",
    ...
    def validate_export(
        ...
        checks.append(ExportCheckResult(
            ...
            passed=path.exists(),
            severity="error",
            message=f"Unreal sidecar not found: {path}",
        ))
        ...
        if path.exists():
            try:
                ...
                checks.append(ExportCheckResult(
                    name="correct_format",
                    passed=data.get("format") == "humanforge-unreal-sidecar",
                    severity="error",
...
register_destination(UnrealDestinationAdapter())
